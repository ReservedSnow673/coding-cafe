...
const StarRating = ({ rating }: { rating: number }) => {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <span key={star} className={star <= rating ? "text-accent-lime" : "text-dark-secondary"}>
          ★
        </span>
      ))}
    </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-4xl font-bold mb-2">Mess Reviews</h1>
              <p className="text-gray-400">Share your feedback about mess meals</p>
            </div>
            <Link
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
            <div className="bg-dark-secondary rounded-lg p-4 text-center border border-dark-secondary">
              <div className="text-sm text-gray-400 mb-1">Overall Today</div>
              <div className="text-3xl font-bold text-accent-lime">{todayAverages.overall || "N/A"}</div>
              <StarRating rating={Math.round(todayAverages.overall)} />
            </div>
            {mealTypes.map((meal) => {
              const Icon = mealTypeIcons[meal];
              return (
                <div key={meal} className="bg-dark-secondary rounded-lg p-4 text-center border border-dark-secondary">
                  <Icon className="w-6 h-6 mx-auto mb-1 text-accent-lime" />
                  <div className="text-sm text-gray-400 capitalize mb-1">{meal}</div>
                  <div className="text-2xl font-bold text-accent-lime">{todayAverages[meal] || "N/A"}</div>
                </div>
              );
            })}
          </div>
        ...
        <div className="space-y-4">
          {filteredReviews.length === 0 ? (
            <div className="bg-dark-secondary rounded-lg p-12 text-center border border-dark-secondary">
              ...
              <p className="text-gray-400 mb-4">Be the first to review this meal!</p>
              <Link
                href="/mess-reviews/new"
            ...
            filteredReviews.map((review) => {
              ...
              return (
                <div key={review.id} className="bg-dark-secondary rounded-lg p-6 border border-dark-secondary hover:border-accent-lime/50 transition-all">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <div className="flex items-center gap-2 text-sm text-gray-400 capitalize bg-dark px-3 py-1 rounded-full">
                  ...
                  <div className="text-3xl font-bold text-accent-lime">{review.rating.toFixed(1)}</div>
                </div>

                {review.review_text && (
                  <p className="text-gray-300 mb-4 ml-16">{review.review_text}</p>
                )}
                  ...
                  <div className="ml-16 grid grid-cols-2 md:grid-cols-4 gap-3">
                    {review.taste_rating && (
                      <div className="bg-dark rounded-lg p-3">
                        <div className="text-xs text-gray-400 mb-1">Taste</div>
                        <div className="flex items-center gap-2">
                          <span className="text-accent-lime font-semibold">{review.taste_rating}</span>
                          <div className="flex-1 h-2 bg-dark-secondary rounded-full overflow-hidden">
                            <div className="h-full bg-accent-lime" style={{ width: `${(review.taste_rating / 5) * 100}%` }} />
                          </div>
                        </div>
                      </div>
                    )}
                    {review.quantity_rating && (
                      <div className="bg-dark rounded-lg p-3">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-accent-lime font-semibold">{review.quantity_rating}</span>
                          <div className="flex-1 h-2 bg-dark-secondary rounded-full overflow-hidden">
                            <div className="h-full bg-accent-lime" style={{ width: `${(review.quantity_rating / 5) * 100}%` }} />
                          </div>
                        </div>
                      </div>
                    )}
                    {review.hygiene_rating && (
                      <div className="bg-dark rounded-lg p-3">
                        <div className="text-xs text-gray-400 mb-1">Hygiene</div>
                        <div className="flex items-center gap-2">
                          <span className="text-accent-lime font-semibold">{review.hygiene_rating}</span>
                          <div className="flex-1 h-2 bg-dark-secondary rounded-full overflow-hidden">
                            <div className="h-full bg-accent-lime" style={{ width: `${(review.hygiene_rating / 5) * 100}%` }} />
                          </div>
                        </div>
                      </div>
                    )}
                    {review.variety_rating && (
                      <div className="bg-dark rounded-lg p-3">
                        <div className="text-xs text-gray-400 mb-1">Variety</div>
                        <div className="flex items-center gap-2">
                          <span className="text-accent-lime font-semibold">{review.variety_rating}</span>
                          <div className="flex-1 h-2 bg-dark-secondary rounded-full overflow-hidden">
                            <div className="h-full bg-accent-lime" style={{ width: `${(review.variety_rating / 5) * 100}%` }} />
                          </div>
                        </div>
                      </div>
                    )}
